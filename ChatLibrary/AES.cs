using System.Security.Cryptography;
using System.Text;

namespace ChatLibrary
{
    public class AES
    {
        private static int Nb, Nk, Nr;
        private static byte[] Sbox = {  };

        private static byte[] InvSbox = {  };

        private static byte[] Rcon = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };

        private static byte[] xor_arrays(byte[] arr1, byte[] arr2)
        {
            byte[] res = new byte[arr1.Length];
            for (int i = 0; i < arr1.Length; i++)
                res[i] = (byte)(arr1[i] ^ arr2[i]);
            return res;
        }

        private static void SubBytes(byte[,] state)
        {
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    state[i, j] = Sbox[(state[i, j] >> 4) * 16 + (state[i, j] & 0x0F)];
        }

        private static void InvSubBytes(byte[,] state)
        {
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    state[i, j] = InvSbox[(state[i, j] >> 4) * 16 + (state[i, j] & 0x0F)];
        }

        private static void SubWord(byte[] word)
        {
            for (int i = 0; i < 4; i++)
                word[i] = Sbox[(word[i] >> 4) * 16 + (word[i] & 0x0F)];
        }

        private static void RotWord(byte[] word)
        {
            byte temp = word[0];
            for (int i = 0; i < 3; i++)
                word[i] = word[i + 1];
            word[3] = temp;
        }

        public static void ShiftRows(byte[,] state)
        {
            for (int row = 1; row < 4; row++)
            {
                byte[] temp = new byte[4];
                for (int col = 0; col < 4; col++)
                    temp[col] = state[row, (col + row) % Nb];
                for (int col = 0; col < 4; col++)
                    state[row, col] = temp[col];
            }
        }

        private static void InvShiftRows(byte[,] state)
        {
            byte[] t = new byte[4];
            for (int r = 1; r < 4; r++)
            {
                for (int c = 0; c < Nb; c++)
                    t[(c + r) % Nb] = state[r, c];
                for (int c = 0; c < Nb; c++)
                    state[r, c] = t[c];
            }
        }

        public static void MixColumns(byte[,] state)
        {
            byte[,] result = new byte[4, 4];
            for (int col = 0; col < 4; col++)
            {
                result[0, col] = (byte)(Multiply(0x02, state[0, col]) ^
                    Multiply(0x03, state[1, col]) ^ state[2, col] ^ state[3, col]);
                result[1, col] = (byte)(state[0, col] ^ Multiply(0x02, state[1, col]) ^
                    Multiply(0x03, state[2, col]) ^ state[3, col]);
                result[2, col] = (byte)(state[0, col] ^ state[1, col] ^
                    Multiply(0x02, state[2, col]) ^ Multiply(0x03, state[3, col]));
                result[3, col] = (byte)(Multiply(0x03, state[0, col]) ^
                    state[1, col] ^ state[2, col] ^ Multiply(0x02, state[3, col]));
            }
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    state[i, j] = result[i, j];
        }

        private static void InvMixColumns(byte[,] state)
        {
            byte[,] result = new byte[4, 4];
            for (int col = 0; col < 4; col++)
            {
                result[0, col] = (byte)(Multiply(0x0e, state[0, col]) ^
                    Multiply(0x0b, state[1, col]) ^ Multiply(0x0d, state[2, col]) ^ Multiply(0x09, state[3, col]));
                result[1, col] = (byte)(Multiply(0x09, state[0, col]) ^
                    Multiply(0x0e, state[1, col]) ^ Multiply(0x0b, state[2, col]) ^ Multiply(0x0d, state[3, col]));
                result[2, col] = (byte)(Multiply(0x0d, state[0, col]) ^
                    Multiply(0x09, state[1, col]) ^ Multiply(0x0e, state[2, col]) ^ Multiply(0x0b, state[3, col]));
                result[3, col] = (byte)(Multiply(0x0b, state[0, col]) ^
                    Multiply(0x0d, state[1, col]) ^ Multiply(0x09, state[2, col]) ^ Multiply(0x0e, state[3, col]));
            }
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 4; j++)
                    state[i, j] = result[i, j];
        }

        private static byte Multiply(byte a, byte b)
        {
            byte p = 0;
            byte counter;
            byte hi_bit_set;
            for (counter = 0; counter < 8; counter++)
            {
                if ((b & 1) == 1)
                    p ^= a;
                hi_bit_set = (byte)(a & 0x80);
                a <<= 1;
                if (hi_bit_set == 0x80)
                    a ^= 0x1b;
                b >>= 1;
            }
            return p;
        }
        private static void AddRoundKey(byte[,] state, byte[,] w, int round)
        {
            for (int c = 0; c < Nb; c++)
                for (int l = 0; l < 4; l++)
                    state[l, c] = (byte)(state[l, c] ^ w[round * Nb + c, l]);
        }

        private static byte[,] KeyExpansion(byte[] key)
        {
            int len = Nb * (Nr + 1);
            byte[,] w = new byte[Nb * (Nr + 1), 4];
            int i = 0;
            while (i < Nk)
            {
                for (int j = 0; j < 4; j++)
                    w[i, j] = key[4 * i + j];
                i++;
            }
            i = Nk;
            while (i < len)
            {
                byte[] tmp = new byte[4];
                for (int j = 0; j < 4; j++)
                    tmp[j] = w[i - 1, j];
                if (i % Nk == 0)
                {
                    RotWord(tmp);
                    SubWord(tmp);
                    tmp[0] ^= Rcon[i / Nk];
                }
                else if (Nk > 6 && i % Nk == 4)
                    SubWord(tmp);
                byte[] tmp1 = new byte[4];
                for (int j = 0; j < 4; j++)
                    tmp1[j] = w[i - Nk, j];
                byte[] tmp2 = xor_arrays(tmp1, tmp);
                for (int j = 0; j < 4; j++)
                    w[i, j] = tmp2[j];
                i++;
            }
            return w;
        }

        private static byte[,] EncryptBlock(byte[,] state, byte[,] w)
        {

            AddRoundKey(state, w, 0);
            for (int round = 1; round < Nr; round++)
            {
                SubBytes(state);
                ShiftRows(state);
                MixColumns(state);
                AddRoundKey(state, w, round);
            }
            SubBytes(state);
            ShiftRows(state);
            AddRoundKey(state, w, Nr);
            return state;
        }

        private static byte[,] DecryptBlock(byte[,] state, byte[,] w)
        {
            AddRoundKey(state, w, Nr);
            for (int round = Nr - 1; round >= 1; round--)
            {
                InvShiftRows(state);
                InvSubBytes(state);
                AddRoundKey(state, w, round);
                InvMixColumns(state);

            }
            InvShiftRows(state);
            InvSubBytes(state);
            AddRoundKey(state, w, 0);
            return state;
        }
        public static byte[] Encrypt(byte[] message, byte[] key)
        {
            using Aes aes = Aes.Create();
            aes.Key = key;
            aes.Mode = CipherMode.ECB;
            ICryptoTransform encryptor = aes.CreateEncryptor();
            byte[] encrypted = encryptor.TransformFinalBlock(message, 0, message.Length);
            return encrypted;
        }

        public static byte[] Decrypt(byte[] message, byte[] key)
        {
            using Aes aes = Aes.Create();
            aes.Key = key;
            aes.Mode = CipherMode.ECB;
            ICryptoTransform decryptor = aes.CreateDecryptor();
            byte[] decrypted = decryptor.TransformFinalBlock(message, 0, message.Length);
            return decrypted;
        }

        public static byte[] EncryptSlow(byte[] message, byte[] key)
        {
            Nb = 4;
            Nk = key.Length / 4;
            Nr = Nk + 6;
            message = Pad(message);
            int blockCount = message.Length / (Nb * 4);
            byte[,] state = new byte[Nb, 4];
            byte[,] w = KeyExpansion(key);
            byte[] ciphertext = new byte[message.Length];
            for (int b = 0; b < blockCount; b++)
            {
                for (int i = 0; i < Nb; i++)
                    for (int j = 0; j < 4; j++)
                        state[i, j] = message[b * (Nb * 4) + j * Nb + i];
                state = EncryptBlock(state, w);
                for (int i = 0; i < Nb; i++)
                    for (int j = 0; j < 4; j++)
                        ciphertext[b * (Nb * 4) + j * Nb + i] = state[i, j];
            }
            return ciphertext;
        }
        private static byte[] Pad(byte[] input)
        {
            int paddingLength = 16 - input.Length % 16;
            byte[] output = new byte[input.Length + paddingLength];
            Buffer.BlockCopy(input, 0, output, 0, input.Length);
            for (int i = input.Length; i < output.Length; i++)
                output[i] = (byte)paddingLength;
            return output;
        }

        private static byte[] Unpad(byte[] input)
        {
            int paddingLength = input[input.Length - 1];
            byte[] output = new byte[input.Length - paddingLength];
            Buffer.BlockCopy(input, 0, output, 0, output.Length);
            return output;
        }

        public static byte[] DecryptSlow(byte[] message, byte[] key)
        {
            Nb = 4;
            Nk = key.Length / 4;
            Nr = Nk + 6;
            int blockCount = message.Length / (Nb * 4);
            byte[,] state = new byte[Nb, 4];
            byte[,] w = KeyExpansion(key);
            byte[] plaintext = new byte[message.Length];
            for (int b = 0; b < blockCount; b++)
            {
                for (int i = 0; i < 4; i++)
                    for (int j = 0; j < Nb; j++)
                        state[i, j] = message[b * (Nb * 4) + j * Nb + i];
                DecryptBlock(state, w);
                for (int i = 0; i < 4; i++)
                    for (int j = 0; j < Nb; j++)
                        plaintext[b * (Nb * 4) + j * Nb + i] = state[i, j];
            }
            plaintext = Unpad(plaintext);
            return plaintext;
        }
    }
}
